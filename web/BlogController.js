...
function queryHotBlog(request,response){
    blogDao.queryHotBlogBySize(5,function(result){
...
function queryBlogByPage(request, response) {
    var params = url.parse(request.url, true).query;//把传过来的url？号后边参数转化成对象
    blogDao.queryBlogByPage(parseInt(params.page),parseInt(params.pageSize),function(result){
        for(var i = 0; i<result.length;i++){
            result[i].content = result[i].content.replace(/<img[\w\W]*"/,"");//过滤掉img开头的
            result[i].content = result[i].content.replace(/<[\w\W]{1,5}>/g,"");//过滤掉标签
            result[i].content = result[i].content.substring(0,300);//截断字符最多300个
        }
        response.writeHead(200);
        response.write(respUtil.writeResult("success","查询成功",result));
        response.end();
